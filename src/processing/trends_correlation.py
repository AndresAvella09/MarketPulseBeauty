...
def assign_trend_keyword(text: str) -> str | None:
    """
    Mapea cada producto a una de las 3 keywords principales de Trends.
    """
    s = str(text).lower()
    if "niacin" in s:
        return "niacinamida"
...
def main():
    ...
    trends_weekly = (
        trends.groupby(["trend_keyword", "week"], as_index=False)
    ...
    # --- 2) pd_info -> map pd_id -> trend_keyword ---
    ...
    if not text_cols:
        raise ValueError("pd_info.csv no tiene columnas de texto para clasificar (keyword/Name/Description/category).")
    ...
    # --- 3) reviews -> weekly volume por trend_keyword ---
    reviews = pd.read_csv(reviews_path)
    required_rv = {"pd_id", "SubmissionTime"}
    missing_rv = required_rv - set(reviews.columns)
    if missing_rv:
        raise ValueError(f"review_data.csv no tiene columnas {required_rv}. Faltan: {missing_rv}")
    ...
    reviews_weekly = (
        rv.groupby(["trend_keyword", "week"], as_index=False)
        .size()
    ...
    corr = (
        pd.DataFrame(rows).sort_values("trend_keyword")
        if rows else
        pd.DataFrame(columns=["trend_keyword", "n_weeks", "pearson", "spearman"])
    )

    # --- 6) guardar en /data/processed/google_trends ---
    ...
    print("[OK] Outputs guardados en:", out_dir)
    print(corr)
